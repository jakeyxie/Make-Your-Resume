    ...
    <div  class="template-card" >
      <el-space wrap class="card-space" :size="20">
        <div class="cards" @mouseenter="mouseenter(index)" @mouseleave="mouseleave(index)" @click="handleGotoTemplate(item)"
          ...
          <span style="color: rgb(38, 38, 38); font-size: 14px">{{item.time}}</span>

          <!-- 遮罩层 -->
          <div class="mask-layer" v-show="data.hoveredCards[index]">
            <div class="del-icon" title="预览" @click.stop="del(item)">
              <svg-icon iconName="icon-shanchu1" color="#fff" size="17px"></svg-icon>
            </div>
            <div class="design-button" @click="toDesign">立即继续制作</div>
          </div>
        </div>
      </el-space>
...
const del = (data) => {
  ElMessageBox.confirm('删除后无法恢复，您确认删除吗？', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/userTemplate/delete/' + data.id).then(res => {
      if (res.code === '200') {
...
.card-space{
  display: grid;
  grid-template-columns: repeat(4, 130px); /* 四列 */
