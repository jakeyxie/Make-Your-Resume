...
<template>
  <div class="resume-theme-box">
    <el-form id="resume-form" label-width="160px" label-position="left">
...
<script setup>
  ...
  // 二级标题字体大小发生变化时
  const secondTitleFontSizeChange = (value) => {
  ...
  // 正文字体大小发生变化
  const textFontSizeChange = (value) => {
    resumeJsonStore.LIST.forEach((item) => {
  ...
  // 二级标题颜色改变
  const secondTitleColorChange = (value) => {
    resumeJsonStore.LIST.forEach((item) => {
  ...
  // 二级标题字体粗细
  const secondTitleWeightChange = (value) => {
    resumeJsonStore.LIST.forEach((item) => {
  ...
  // 上外边距
  const defaultMtop= pxTonumber(resumeJsonStore.GLOBAL_STYLE.modelMarginTop);
  ...
  // 下外边距
  const defaultMbottom = pxTonumber(resumeJsonStore.GLOBAL_STYLE.modelMarginBottom);
